                ...
                <div id="topattr" style="width: 100%" class="col-md-5">
                    <div style="visibility: hidden;" id="prof" class="card mb-3 shadow-sm">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text" id="userRegistered"></p>
                        </div>
                    </div>
    ...
    if ($isrunned == false) {
        $.ajax({
            ...
            success: function(data) {
                if (data.code != 404) {             //test.php에서 오류 발생 시 404에러로 return
                    document.getElementById("row").style.visibility = "visible";        
                    $dat = data;
                    document.getElementById("load").style.display = "none";
                    var count = data.GNum;
                    var totTime = 0;
                    var registeredDate = new Date(data.registeredIn * 1000);            //Unix Timestame 변환
                    $('#cnt').text(count + "개");
                    $('#test1').attr('src', data.photo);
                    $('#username1').append(data.comuName);
                    $('#userRegistered').append(registeredDate.getFullYear() + "년 " + (registeredDate.getMonth()+1) + "월 "+registeredDate.getDate()+"일 가입");
                    ...
                    $('#timetable>tbody:last').append('<div class="card mb-3 shadow-sm" id="tcard1"><tr><td><p id="tdesc1">최저임금 9160원 기준</p></td></tr><tr><td><div style="overflow: hidden;"> <p id="tdesc2"><i class="fas fa-wallet"></i> ' + numberWithCommas(Math.round(totTime * 152.66)) + '원</p> 벌 수 있었습니다.</div></td></tr></div>');
